...
class MealsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        let expiryDate = df.date(from: UserDefaults.standard.string(forKey: "expiry_date") ?? "") ?? Date()
        
        if token.count == 32 &&  expiryDate > Date() {
            let token = UserDefaults.standard.string(forKey: "token")!
            
            AF.request("http://diyet.atwebpages.com/meal/types/"+token, method: .get).validate().responseJSON { response in
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "MealTypesCell", for: indexPath) as! MealTypesTableViewCell
        // Configure the cell...
        
        cell.mealType.text = self.result[indexPath.row]["title"].stringValue
        
        if self.result[indexPath.row]["created"].boolValue {
            cell.editButton.isHidden = true
            cell.tag = self.result[indexPath.row]["meal_id"].intValue
            cell.showButton.isHidden = false
        }else{
            cell.editButton.isHidden = false
            cell.editButton.setTitle("Oluştur", for: .normal)
            cell.tag = self.result[indexPath.row]["meal_types_id"].intValue
